...
func (iq *IdeaQuery) QueryTranslations() *IdeaTranslationQuery {
	query := (&IdeaTranslationClient{config: iq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := iq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (iq *IdeaQuery) WithTranslations(opts ...func(*IdeaTranslationQuery)) *IdeaQuery {
	query := (&IdeaTranslationClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (iq *IdeaQuery) loadTranslations(ctx context.Context, query *IdeaTranslationQuery, nodes []*Idea, init func(*Idea), assign func(*Idea, *IdeaTranslation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(ideatranslation.FieldIdeaID)
	}
