...
func (clq *CommentLikeQuery) QueryUserIdentity() *UserIdentityQuery {
	query := (&UserIdentityClient{config: clq.config}).Query()
...
func (clq *CommentLikeQuery) WithUserIdentity(opts ...func(*UserIdentityQuery)) *CommentLikeQuery {
	query := (&UserIdentityClient{config: clq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	clq.withUserIdentity = query
	return clq
}
