...
func (cq *CommentQuery) QueryUserIdentity() *UserIdentityQuery {
	query := (&UserIdentityClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CommentQuery) Clone() *CommentQuery {
	...
	return &CommentQuery{
		...
		withUserIdentity: cq.withUserIdentity.Clone(),
...
func (cq *CommentQuery) WithUserIdentity(opts ...func(*UserIdentityQuery)) *CommentQuery {
	query := (&UserIdentityClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withUserIdentity = query
	return cq
}
...
//		GroupBy(comment.FieldEntityType).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Comment.Query().
//		Select(comment.FieldEntityType).
//		Scan(ctx, &v)
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withUserIdentity; query != nil {
		if err := cq.loadUserIdentity(ctx, query, nodes, nil,
			func(n *Comment, e *UserIdentity) { n.Edges.UserIdentity = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommentQuery) loadUserIdentity(ctx context.Context, query *UserIdentityQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *UserIdentity)) error {
...
func (cq *CommentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withUserIdentity != nil {
			_spec.Node.AddColumnOnce(comment.FieldUserIdentityID)
		}
	}
